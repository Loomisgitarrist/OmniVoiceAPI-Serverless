...
def build_demo(
    ...
    test_cases = [
        ("afghanistan", "fa", "salam", None),
        ("bulgaria", "bg", "zdravey", None),
        ("french", "fr", "salut", None),
        ("georgia", "ka", "gamarjoba", None),
        ("germany", "de", "hallo", None),
        ("iran", "fa", "salam", None),
        ("italian", "it", "ciao", None),
        ("poland", "pl", "czesc", None),
        ("romania", "ro", "salut", None),
        ("russian", "ru", "privet", None),
        ("spanish", "es", "hola", None),
        ("syria", "apc", "marhaba", None),
        ("turkey", "tr", "merhaba", None),
        ("ukraine", "uk", "pryvit", None),
        ("united-kingdom", "en", "hello there", "british accent"),
        ("uzbekistan", "uz", "salom", None),
    ]
    ...
    def _run_language_smoke_test(num_step: int):
        out_dir = Path(tempfile.mkdtemp(prefix="omnivoice_lang_test_"))
        failures = []
        ok = 0
        for name, lang, text, instruct in test_cases:
            out_path = out_dir / f"{name}_{lang}.wav"
            try:
                ...
                sf.write(out_path, audio[0], sampling_rate)
                ok += 1
            except Exception as e:
                failures.append((name, lang, f"{type(e).__name__}: {e}"))

        zip_path = out_dir / "voice_test_outputs.zip"
        with zipfile.ZipFile(zip_path, "w", compression=zipfile.ZIP_DEFLATED) as zf:
            for p in sorted(out_dir.glob("*.wav")):
                zf.write(p, arcname=p.name)

        if failures:
            lines = [f"ok={ok} fail={len(failures)}", ""]
            lines.extend([f"- {n} ({l}): {err}" for n, l, err in failures])
            return str(zip_path), "\n".join(lines)

        return str(zip_path), f"ok={ok} fail=0"
            ...
            with gr.TabItem("Language Test"):
                with gr.Row():
                    with gr.Column(scale=1):
                        ...
                        lt_btn = gr.Button(
                            "Run Language Smoke Test (ZIP)", variant="primary"
                        )
                    with gr.Column(scale=1):
                        lt_zip = gr.File(label="Download", file_types=[".zip"])
                        lt_status = gr.Textbox(label="Status", lines=10)

                lt_btn.click(
                    _run_language_smoke_test,
                    inputs=[lt_steps],
                    outputs=[lt_zip, lt_status],
                )
